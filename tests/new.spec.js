...
test('MakeMyTrip',async({page})=>
{
    ...
    await page.locator('.commonModal__close').click();//pop-up
    await page.locator('.menu_Hotels').click();
    await page.locator('#city').click();
    const closeButton=page.locator('.spa-classic-peek__back-cta');
    ...
    //await page.locator('.DayPicker-Day',{hasText:'25'}).nth(1).click();
    //await page.locator('.DayPicker-Day',{hasText:'30'}).nth(1).click();
    await page.locator("div[aria-label='Mon Jun 08 2026']").last().click();
    await page.locator("div[aria-label='Fri Jun 12 2026']").last().click();
    ...
    await page.locator("//div[@class='w-6 h-6 bg-white rounded-full flex items-center justify-center p-0.5']").click();//popup
    ...
    await page.locator('#hlistpg_proptypes_show_more').nth(2).click();//show more
    await page.locator('.makeFlex.column',{hasText:'Balcony/Terrace'}).click();//balcony
    await page.locator('.rangeInput').first().click();
    await page.locator('.rangeInput').first().fill('10000');
    await page.locator('.rangeInput').nth(1).click();
    await page.locator('.rangeInput').nth(1).fill('25000');
    await page.locator('.btnRangeGo.appendLeft5').click();//apply
    ...
    for (let i = 0; i < 3; i++)
    {
        ...
        const rate=await newpage.locator('.ratingGrph__percent.no-translate.notranslate').first().textContent();//top rating percentage
        await newpage.locator('.ratingGrph.mmt').first().screenshot({path:`rating${i}.png`});//rating screenshot
        const rating=Number(rate.replace(/[^\d]/g, ""));//% to number
        await console.log(n);
    ...
    const [npage] = await Promise.all
        ([
            page.context().waitForEvent('page'),
            await page.locator('.wordBreak.appendRight10',{hasText:name}).click()//click on the top rated hotel
        ]);
    await npage.waitForLoadState();
    await npage.locator('.rmPayable__newDtl--right.newDtls').nth(0).click();
    await npage.locator('.cpnCodeCard').first().waitFor();
    ...
    await npage.locator('.dlCodes.appendBottom20').screenshot({path:'BankOffers.png'});
